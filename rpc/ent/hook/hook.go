...
// The AmisSchemaFunc type is an adapter to allow the use of ordinary
// function as AmisSchema mutator.
type AmisSchemaFunc func(context.Context, *ent.AmisSchemaMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f AmisSchemaFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.AmisSchemaMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.AmisSchemaMutation", m)
	}
	return f(ctx, mv)
}
