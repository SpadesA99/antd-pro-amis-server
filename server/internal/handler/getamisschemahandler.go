...
func GetAmisSchemaHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		type ErrorReply struct {
			Status  int    `json:"status"`
			Message string `json:"message"`
		}
		var req types.GetAmisSchemaRequest
		...
		l := logic.NewGetAmisSchemaLogic(r.Context(), svcCtx)
		resp, err := l.GetAmisSchema(&req)
		if err != nil {
			w.WriteHeader(http.StatusOK)
			w.Header().Set("Content-Type", "application/json")
			res, _ := json.Marshal(&ErrorReply{Status: 400, Message: err.Error()})
